...
export const getNextEvent = (races?: Race[]): NextEvent | undefined => {
  ...
  for (const race of races) {
    ...
    if (
      ...
      race.events.firstPractice.dateTime.diff(now, ["seconds"]).seconds > 0
    ) {
      return {
    ...
    if (!race.events.isSprintWeekend) {
      if (
        race.events.secondPractice &&
        race.events.secondPractice.dateTime.diff(now, ["seconds"]).seconds > 0
      ) {
        return {
        ...
        race.events.thirdPractice &&
        race.events.thirdPractice.dateTime.diff(now, ["seconds"]).seconds > 0
      ) {
        return {
        ...
        race.events.qualifying &&
        race.events.qualifying.dateTime.diff(now, ["seconds"]).seconds > 0
      ) {
        return {
      ...
      if (
        race.events.sprintShootout &&
        race.events.sprintShootout.dateTime.diff(now, ["seconds"]).seconds > 0
      ) {
        return {
      ...
      if (
        race.events.sprint &&
        race.events.sprint.dateTime.diff(now, ["seconds"]).seconds > 0
      ) {
        return {
